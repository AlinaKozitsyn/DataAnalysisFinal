...
# fig2: histograms of the three main scales
...
# fig4/fig5: TotalIA against depression and loneliness, faceted by cluster
...
SAVE("fig4_iat_vs_depression_by_cluster.png",
     scatter_by_cluster("totalphq", "Depression (PHQ-9, 0–27)", "Fig 4",
       "Steepest depression slope among males who don't exercise; flattest among females who exercise"))
SAVE("fig5_iat_vs_loneliness_by_cluster.png",
     scatter_by_cluster("lonelinesstotal", "Loneliness (UCLA, 29–75)", "Fig 5",
       "Loneliness adds little once depression is accounted for (see Fig 6)"))

# fig6: standardized slopes per cluster
p6 <- cl_coef_z %>% mutate(cluster = factor(cluster, levels = rev(clusters))) %>%
  ...
  labs(title = "Fig 6 — Depression drives internet addiction in every cluster; loneliness does not",
...
SAVE("fig6_cluster_coefficients.png", p6, h = 5.0)

# fig7: residual diagnostics for the pooled model
...
p7 <- (pa | pb) +
  plot_annotation(title = "Fig 7 — Model diagnostics: roughly linear, with a mild right-tail in the residuals",
                  ...
                  theme = theme(plot.title = element_text(face = "bold", size = 14),
                                plot.caption = element_text(color = "grey45", size = 8.5, hjust = 0)))
SAVE("fig7_residual_diagnostics.png", p7, w = 10.5, h = 4.8)

# fig8: addiction rate by cluster
p8 <- cluster_profile %>% mutate(group4 = factor(group4, levels = clusters)) %>%
  ...
  labs(title = "Fig 8 — Internet-addiction prevalence more than doubles across clusters",
...
SAVE("fig8_addiction_prevalence_by_cluster.png", p8, h = 5.0)

# fig9: ROC curve for the forecast model
p9 <- ggplot(roc_df, aes(fpr, tpr)) +
  ...
  coord_equal() +
  labs(title = "Fig 9 — Forecast discrimination (held-out test set)",
...
SAVE("fig9_roc.png", p9, w = 7, h = 6.4)

# fig10: predicted probabilities split by true class
p10 <- test %>% mutate(actual = factor(addicted, c(0,1), c("Not addicted","Addicted"))) %>%
  ...
  labs(title = "Fig 10 — Higher predicted probabilities for truly-addicted students",
